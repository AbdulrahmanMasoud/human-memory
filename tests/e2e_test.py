...
def test_store_memory_5():
    r = client.post("/v1/memories", json={"content": "Docker containers use Alpine Linux for small images"})
    assert r.status_code == 201
    memory_ids.append(r.json()["memory_id"])
...
def test_create_concept_python():
    r = client.post("/v1/graph/concepts", json={"name": "Python", "type": "language", "activation": 0.9})
    assert r.status_code == 201
    data = r.json()
...
def test_create_relation_python_ai():
    r = client.post("/v1/graph/relations", json={
...
def test_create_relation_fastapi_python():
    r = client.post("/v1/graph/relations", json={
        ...
        "relation_type": "WORKS_WITH", "weight": 0.95
    })
    assert r.status_code == 201

def test_create_relation_fastapi_webdev():
    r = client.post("/v1/graph/relations", json={
        "source": "FastAPI", "target": "Web Development",
        "relation_type": "USED_FOR", "weight": 0.85
    })
    assert r.status_code == 201
...
def test_spreading_activation():
    r = client.post("/v1/graph/search", json={
        "active_concepts": ["Python"],
        "depth": 2,
        "limit": 10
    })
    assert r.status_code == 200
...
def test_spreading_activation_multi():
    r = client.post("/v1/graph/search", json={
        "active_concepts": ["Python", "FastAPI"],
        "depth": 2,
        "limit": 10
    })
    assert r.status_code == 200
...
def test_working_memory_capacity():
    ...
    for i in range(10):
        client.post("/v1/memories", json={
            "content": f"Test memory number {i} about various topics"
        })
...
def test_forget_strategic():
    r = client.post("/v1/memories/forget-strategy", json={
...
def test_actr_frequency():
    ...
    many = actr.base_level_activation([now - 3600, now - 3000, now - 2400, now - 1800, now - 1200], now)
...
def test_actr_spreading():
    ...
    s = actr.spreading_activation(
        "target",
        ["ctx1", "ctx2"],
        {("ctx1", "target"): 2.0, ("ctx2", "target"): 1.0}
    )
...
def test_clustering_basic():
    from memory_system.core.clustering import cluster_episodes, cosine_similarity
...
def test_full_lifecycle():
    ...
    initial_activation = r.json()["activation"]
